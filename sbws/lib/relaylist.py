...
class Relay:
    ...
    def _set_new_consensus_timestamps(self, timestamp=None):
        # It has been already updated or there's no timestamp yet
        if self.consensus_valid_after is not None:
            if self.last_consensus is not None:
                if self.consensus_valid_after > self.last_consensus:
                    self._consensus_timestamps.append(
                        self.consensus_valid_after
                        )
            else:
                self._consensus_timestamps.append(self.consensus_valid_after)
        elif (self.last_consensus is not None
              and timestamp > self.last_consensus):
