...
class V3BWLine(object):
    ...
    def __init__(self, node_id, bw, **kwargs):
        ...
        self.bw = bw
        # To do not add ``bw_filt`` to the bandwidth file, but be able to
        # initialize a line with it.
        [setattr(self, k, v) for k, v in kwargs.items()
        ...
        if router_statuses_d and node_id in router_statuses_d:
            ...
            consensus_bandwidth_is_unmeasured = \
                cls.consensus_bandwidth_is_unmeasured_from_results(
                    results_recent)
        # If there is not last observed bandwidth, there won't be mean either.
        desc_bw_obs_last = \
...
class V3BWFile(object):
    ...
    def from_results(cls, results, scanner_country=None,
        ...
        # XXX: Use router_statuses_d to do not parse again the faile.
        number_consensus_relays = \
    ...
    def bw_torflow_scale(bw_lines, desc_bw_obs_type=TORFLOW_OBS_MEAN,
        ...
        **Limit the bandwidth to a maximum**::
        ...
        for l in bw_lines_tf:
            ...
            # If the observed bandwidth is None, it i not possible to
            # calculate the minimum with the other descriptors.
            # Only in this case, take only the consenus bandwidth.
            # In the case that descriptor average or burst are None,
            # ignore them since it must be a bug, but scale.
            ...
            # Asign it to an attribute, so it's not lost before capping and
            # rounding
